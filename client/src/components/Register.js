...
const Register = () => {
  ...
  const Postdata = async (e) => {
    ...
    if (!name || !email || !work || !password || !cpassword) {
      ...
      window.alert("code is not correct");
    } else {
      try {
        const formData = new FormData();
        formData.append("name", name);
        formData.append("email", email);
        formData.append("code", work);
        formData.append("password", password);
        formData.append("cpassword", cpassword);

        const res = await axios.post(apiUrl + "/registe/coet", formData, {
          withCredentials: true,
        });

        // console.log("regsiter college response", res.status);
        if (res.status == 201) {
        ...
        console.log("error register college", error);
      }
    }
  };
