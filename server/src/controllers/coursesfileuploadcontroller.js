...
const FlutterURLUploadController = async (req, res, next) => {
  const final_path = req.file.path;

  const base64 = fs.readFileSync(final_path, "base64");
  const buffer = Buffer.from(base64, "base64");
  try {
    const multipleFiles = new FlutterUrl({
      url: req.body.url,
      image: {
        data: buffer,
        contentType: req.file.mimetype,
...
const ReactnativeURLUploadController = async (req, res, next) => {
  const final_path = req.file.path;

  const base64 = fs.readFileSync(final_path, "base64");
  const buffer = Buffer.from(base64, "base64");
  try {
    const multipleFiles = new ReactNativeUrl({
      url: req.body.url,
      image: {
        data: buffer,
        contentType: req.file.mimetype,
...
const UnityURLUploadController = async (req, res, next) => {
  const final_path = req.file.path;

  const base64 = fs.readFileSync(final_path, "base64");
  const buffer = Buffer.from(base64, "base64");
  try {
    const multipleFiles = new UnityUrl({
      url: req.body.url,
      image: {
        data: buffer,
        contentType: req.file.mimetype,
