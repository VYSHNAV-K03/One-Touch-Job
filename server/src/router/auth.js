...
router.post("/registe/coet", upload.any(), async (req, res) => {
  ...
  try {
    ...
    if (userExist) {
      res.status(200).send("Email is already present");
    } else if (password === cpassword) {
      const user = new COET({
        name,
        email,
      ...
      res.status(200).send("password must be equal");
    }
  } catch (e) {
...
router.post("/updatepassion/off/:passion", Authenticate, async (req, res) => {
  try {
    const user = await User.updateOne(
      {
        _id: req.userID,
